...
const PyroHandler = require('../PyroHandler');
 ...
 * @extends {PyroHandler}
 */
class CommandHandler extends PyroHandler {
    constructor(client, {
        directory,
        ...
        if (!(classToHandle.prototype instanceof Command || classToHandle === Command)) {
            throw new PyroError('INVALID_CLASS_TO_HANDLE', classToHandle.name, Command.name);
        }
        ...
        if ((this.handleEdits || this.storeMessages) && !this.commandUtil) {
            throw new PyroError('COMMAND_UTIL_EXPLICIT');
        }
    ...
    register(command, filepath) {
        ...
        for (let alias of command.aliases) {
            ...
            if (conflict) throw new PyroError('ALIAS_CONFLICT', alias, command.id, conflict);
            ...
            if (this.aliasReplacement) {
                ...
                if (replacement !== alias) {
                    ...
                    if (replacementConflict) throw new PyroError('ALIAS_CONFLICT', replacement, command.id, replacementConflict);
...
/**
 * Also includes properties from PyroHandlerOptions.
 * @typedef {PyroHandlerOptions} CommandHandlerOptions

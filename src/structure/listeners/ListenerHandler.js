 ...
 * @param {PyroHandlerOptions} options - Options.
 * @extends {PyroHandler}
 */
class ListenerHandler extends PyroHandler {
    constructor(client, {
        directory,
        ...
        if (!(classToHandle.prototype instanceof Listener || classToHandle === Listener)) {
            throw new PyroError('INVALID_CLASS_TO_HANDLE', classToHandle.name, Listener.name);
        }
    ...
    addToEmitter(id) {
        ...
        if (!listener) throw new PyroError('MODULE_NOT_FOUND', this.classToHandle.name, id);

        const emitter = isEventEmitter(listener.emitter) ? listener.emitter : this.emitters.get(listener.emitter);
        if (!isEventEmitter(emitter)) throw new PyroError('INVALID_TYPE', 'emitter', 'EventEmitter', true);
    ...
    setEmitters(emitters) {
        for (const [key, value] of Object.entries(emitters)) {
            if (!isEventEmitter(value)) throw new PyroError('INVALID_TYPE', key, 'EventEmitter', true);
